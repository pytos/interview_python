...
`_foo`:一种约定,表示这是一个保护成员.不能用from module import * 导入，如果使用import module的方式导入模块,仍然可以以module._x 的方式访问对象，其他方面和公有一样访问；

`__foo`:双下划线开头，表示私有成员，只允许类本身访问，子类也不能访问。解析器用`_classname__foo`来代替这个名字,以区别和其他类相同的命名,它无法直接像公有成员一样随便访问,通过对象名._类名__xxx这样的方式可以访问.
